								  ...
								  const int inputChannels)
{
	...
	if((dx<outputWidth) && (dy<outputHeight))
	{
		...
		if(inputChannels==1) { // grayscale image
			...
			int isx2 = min(isx1+1, src.cols-1);
			int isy2 = min(isy1+1, src.rows-1);
			...
			float blue = (a[2])*((x2 - src_x) * (y2 - src_y)) + (b[2])*((x2 - src_x) * (y2 - src_y)) + (c[2])*((x2 - src_x) * (y2 - src_y)) + (d[2]) * ((x2 - src_x) * (y2 - src_y));

			float green = (a[1])*((x2 - src_x) * (y2 - src_y)) + (b[1])*((x2 - src_x) * (y2 - src_y)) + (c[1])*((x2 - src_x) * (y2 - src_y)) + (d[1]) * ((x2 - src_x) * (y2 - src_y));

			float red = (a[0])*((x2 - src_x) * (y2 - src_y)) + (b[0])*((x2 - src_x) * (y2 - src_y)) + (c[0])*((x2 - src_x) * (y2 - src_y)) + (d[0]) * ((x2 - src_x) * (y2 - src_y));

			int outputindex = outputYIndex * outputWidth * inputChannels + (outputXIndex * inputChannels);
...
void downscaleCuda(const cv::Mat& input, cv::Mat& output)
{
	...
	resizeCudaKernel<<<grid,block>>>(d_input, d_output, output.cols, output.rows, input.cols, output.rows, pixelGroupSizeX, pixelGroupSizeY, input.channels());
